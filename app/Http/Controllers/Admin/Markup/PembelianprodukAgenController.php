...
use App\AppModel\V_pembelianproduk_agen;
use App\AppModel\Pembelian_markup;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use DB;

class PembelianprodukAgenController extends Controller
{
    public function index()
    {
        $produksWeb     = V_pembelianproduk_agen::all();
        $produksMobile  = V_pembelianproduk_agen::paginate(10);

        return view('admin.markup.pembelian.agen.index', compact('produksWeb', 'produksMobile'));
    }
    ...
    public function updateHargaOtomatis(Request $request)
    {
        ...
        $produk = Pulsa::produk_pembelian($code);
    ...
    public function updateHargaSemua(Request $request)
    {
        DB::beginTransaction();
        try{
            $update = Pembelian_markup::where('markup_agen','<>',0)->update(['markup_agen' => 0]);
            
            DB::commit();
            if($update){
                return response()->json([
                    'success'=> true, 
                    'message'=> 'Reset all markup Agen success!', 
                ], 200);
            }else{
                return response()->json([
                    'success'=> false, 
                    'message'=> 'Reset all markup Agen failed!', 
                ], 200);
            }
        }catch(\Exception $e){
            DB::rollback();
    ...
    public function updateHaragSumMakupPerKategori(Request $request)
    {
        ...
        try{
            ...
            foreach($header_trans as $data){
                 Pembelian_markup::where('id_product', $data->id)->update(['markup_agen'  => DB::raw('markup_agen '.$aksi.' '.intval($nominal).'')]);
            }

            DB::commit();
            return response()->json([
                'success'=> true, 
                'message'=> ''.$aksi.' '.$request->nominal.' markup Agen success!', 
            ], 200);
        }catch(\Exception $e){
            DB::rollback();
            return response()->json([
   ...
   //update data by operator
        ...
        try{
            ...
            foreach($header_trans as $data){
                ...
                foreach($header_trans as $data_trans){
                    Pembelian_markup::where('id_product', $data_trans->id)->update(['markup_agen'  => DB::raw('markup_agen '.$aksi.' '.intval($nominal).'')]);
                }
            }
            DB::commit();
            return response()->json([
    ...
    public function updateHargaPerKategori(Request $request)
    {
        ...
        try{
            ...
            foreach($getPembelian as $data){
                Pembelian_markup::where('id_product', $data->id)->where('markup_agen','<>',0)->update(['markup_agen' => 0]);
            }

            DB::commit();
            return response()->json([
                'success'=> true, 
                'message'=> 'Reset all markup Agen By Kategori success!', 
            ], 200);
        }catch(\Exception $e){
            DB::rollback();
            return response()->json([
    ...
    public function updateHargaPerOperator(Request $request)
    {
        ...
        try{
            ...
            foreach($query as $data){
                ...
                foreach($header_trans as $data_trans){
                    Pembelian_markup::where('id_product', $data_trans->id)->where('markup_agen','<>',0)->update(['markup_agen' => 0]);
                }
            }
            DB::commit();
            return response()->json([
                'success'=> true, 
                'message'=> 'Reset all markup Agen By Operator success!', 
            ], 200);
        }catch(\Exception $e){
            DB::rollback();
            return response()->json([
    ...
    public function showbyKategori($title)
    {
        ...
        return view('admin.markup.pembelian.agen.show', compact('kategori','operator','kategori_all'));
    }
    ...
    {   
        ...
        try{
            ...
            DB::table('pembelian_markups')->update(['markup_agen'  => DB::raw('markup_agen '.$aksi.' '.intval($nominal).'')]);
            DB::commit();
            return response()->json([
                'success'=> true, 
                'message'=> ''.$aksi.' '.$request->nominal.' markup Agen success!', 
    ...
    {
        $produks  = V_pembelianproduk_agen::where('id',$id)->first();
        ...
        if($produk->success == false ){
            ...
            $results[] = [
                    ...
                    // 'operator'     =>$produks->brand,
                    'price_api'    =>$produk->price,
                    'status'       =>$produk->status,
            ];

            return view('admin.markup.pembelian.agen.edit', compact('produks','results','operator'));
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $getPembelian = Pembelian_markup::where('id_product', $id)
                        ->update([
                            'markup_agen' => str_replace('.','', $request->price_markup),
                        ]);
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('admin.produkAgen.index')->with('alert-success', 'Berhasil Menghapus Data Produk');
    }
}
