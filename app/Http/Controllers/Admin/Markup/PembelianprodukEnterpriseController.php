...
class PembelianprodukEnterpriseController extends Controller
{
    ...
    public function updateHargaSemua(Request $request)
    {
        DB::beginTransaction();
        try{
            $update = Pembelian_markup::where('markup_enterprise','<>',0)->update(['markup_enterprise' => 0]);
    ...
    public function updateHaragSumMakupPerKategori(Request $request)
    {
        ...
        try{
            ...
            foreach($header_trans as $data){
                 Pembelian_markup::where('id_product', $data->id)->update(['markup_enterprise'  => DB::raw('markup_enterprise '.$aksi.' '.intval($nominal).'')]);
            }
        ...
        try{
            ...
            foreach($header_trans as $data){
                ...
                foreach($header_trans as $data_trans){
                    Pembelian_markup::where('id_product', $data_trans->id)->update(['markup_enterprise'  => DB::raw('markup_enterprise '.$aksi.' '.intval($nominal).'')]);
                }
            }
            DB::commit();
            return response()->json([
    ...
    public function updateHargaPerKategori(Request $request)
    {
        ...
        try{
            ...
            foreach($getPembelian as $data){
                Pembelian_markup::where('id_product', $data->id)->where('markup_enterprise','<>',0)->update(['markup_enterprise' => 0]);
            }
    ...
    public function updateHargaPerOperator(Request $request)
    {
        ...
        try{
            ...
            foreach($query as $data){
                $header_trans = Pembelianproduk::select('id')->where('pembelianoperator_id', $data->id)->get();
                foreach($header_trans as $data_trans){
                    Pembelian_markup::where('id_product', $data_trans->id)->where('markup_enterprise','<>',0)->update(['markup_enterprise' => 0]);
                }
            }
            DB::commit();
            return response()->json([
    ...
    {   
        ...
        try{
            ...
            DB::table('pembelian_markups')->update(['markup_enterprise'  => DB::raw('markup_enterprise '.$aksi.' '.intval($nominal).'')]);
    ...
    public function edit($jenis,$id)
    {
        ...
        if($produk->success == false ){
            ...
            return view('admin.markup.pembelian.enterprise.edit', compact('produks','results','operator'));
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $getPembelian = Pembelian_markup::where('id_product', $id)
                        ->update([
                            'markup_enterprise' => str_replace('.','', $request->price_markup),
                        ]);
