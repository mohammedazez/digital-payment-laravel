...
<ol class="breadcrumb">
  <li><a href="{{url('/admin')}}" class="btn-loading"><i class="fa fa-dashboard"></i> Home</a></li>
    ...
    <li><a href="{{url('/admin/pembelian-produk/markup/role-personal')}}"> Produk</a></li>
                       ...
                       <div class="box">
                          <div class="box-header with-border">
                             ...
                             <div class="box-tools pull-right">
                                  <a href="{{url('/admin/pembelian-produk/markup/role-personal')}}" class="btn btn-primary btn-sm">Home Produk</a>
                              </div>
                          </div><!-- /.box-header -->
                          ...
                          <div style="text-align: center;margin: 5px;">
                              <div id="desktop" class="hidden-xs hidden-sm">
                                  ...
                                  <a href="{{url('/admin/pembelian-produk/markup/role-personal', $data->slug)}}" class="btn-loading btn btn-primary {{ url('/admin/pembelian-produk/markup/role-personal', $data->slug) == request()->url() ? 'active' : '' }}" style="padding: 3px 7px;margin: 2px">{{$data->product_name}}</a>
                                  @endforeach
                              </div>
                          </div>
                          ...
                          <h4 style="font-weight: bold;text-align: center;">{{$data->product_name}}</h4>
                            <div class="box-body table-responsive no-padding">
                               ...
                               <thead>
                                  ...
                                  @foreach($data->V_pembelianproduk_personal as $produk)
                                  <tr style="font-size: 13px;">
                                     ...
                                     @endif
                                     <td>
                                        <a href="{{url('/admin/pembelian-produk/markup/role-personal/'.$kategori->slug.'/edit', $produk->id)}}" class="btn-loading btn btn-primary btn-sm" style="padding: 2px 5px;font-size: 10px;"><i class="fa fa-pencil-square-o fa-fw"></i>Ubah Harga</a>
                                     </td>
                                  </tr>
                                  @endforeach
                                  @else
                      ...
                      <div class="col-xs-12">
                          <div class="box">
                              <div class="box-header">
                                  <h3 class="box-title" style="font-size:18px;"><a href="{{url('/admin/pembelian-produk/markup/role-personal')}}" class="hidden-lg btn-loading"><i class="fa fa-arrow-left" style="margin-right:10px;"></i></a>{{$kategori->product_name}}</h3>
                             <div class="box-tools pull-right">
                              ...
                              <div style="text-align: center;margin-bottom: 10px;margin-top: 10px;">
                                  <div id="mobile" class="hidden-lg hidden-md">
                                      @foreach($KategoriPembelian as $data)
                                      <a href="{{url('/admin/pembelian-produk/markup/role-personal', $data->slug)}}" class="btn-loading btn btn-primary {{ url('/admin/pembelian-produk/markup/role-personal', $data->slug) == request()->url() ? 'active' : '' }}" style="width:30px;padding: 3px 7px;"><i class="fa fa-{{$data->icon}}"></i><span class="hidden-xs hidden-sm" style="margin-left:5px;">{{$data->product_name}}</span></a>
                                      @endforeach
                                  </div>
                              </div>
                              @if(count($kategori->pembelianoperator) > 0)
                                  ...
                                  <table class="table table-hover">
                                      ...
                                      <tr>
                                          ...
                                          <td align="right" style="width:35%;">
                                              ...
                                              @endif
                                              <div style="margin-top:5px;"><a href="{{url('/admin/pembelian-produk/markup/role-personal/'.$kategori->slug.'/edit', $produk->id)}}" class="btn-loading btn btn-primary btn-sm" style="padding: 2px 5px;font-size: 10px;">Ubah Harga</a></div>
                                          </td>
                                      </tr>
                                      @endforeach
                                      @else
...
// $(document).ready(function() {
    ...
    $('#proses').on('click', function(){
       ...
       $.blockUI({ css: { 
        ...
        if(val_radio =='option_data'){
          $.ajax({
              type: "GET",
              url: '/admin/pembelian-produk/markup/role-personal/plusminusmarkup?aksi='+aksi+'&nominal='+nominal+'type={{$kategori->id}}',
            ...
            $.ajax({
                type: "POST",
                url: "/admin/pembelian-produk/markup/role-personal/plusminusmarkupAllData",
    ...
    {    
       ...
       $.ajax({
            type: "GET",
            url: "{{ url('admin/pembelian-produk/markup/role-personal/cek-harga') }}",
    ...
    function updateHargaSemua()
    {
      ...
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "GET",
            url: "{{ url('admin/pembelian-produk/markup/role-personal/update-harga-semua') }}",
            success: function(response){
              if(response.success == true)
              {
                  $.unblockUI();
    ...
    function updateHargaPerOperator($kategori)
    {
      ...
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "GET",
            url: "/admin/pembelian-produk/markup/role-personal/update-harga-peroperator?id_operator="+id_operator+"&id_kategori="+id_kategori+"",
            success: function(response){
              if(response.success == true)
              {
                  $.unblockUI();
    ...
    function updateHargaPerKategori(){
      ...
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "GET",
            url: "/admin/pembelian-produk/markup/role-personal/update-harga-perkategori?id_kategori="+id_kategori+"",
            success: function(response){
              if(response.success == true)
              {
                  $.unblockUI();
    ...
    function sumMarkingByKategori()
    {
      ...
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "POST",
            url: "{{ url('admin/pembelian-produk/markup/role-personal/update-harga-sum-markup-bykategori') }}",
    ...
    function sumMarkingByOperator()
    {
      ...
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "POST",
            url: "{{ url('admin/pembelian-produk/markup/role-personal/update-harga-sum-markup-byoperator') }}",
  ...
  $('#pilih_kategori').on('change', function(e){
   ...
   $.ajaxSetup({
        ...
        $.ajax({
            url: '/admin/pembelian-produk/markup/role-personal/process-cari/findproduct',
            dataType: "json",
            type: "GET",
            data: {
