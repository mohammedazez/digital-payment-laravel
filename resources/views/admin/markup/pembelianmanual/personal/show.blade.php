...
<h1>Produk <small>{{$kategori->product_name}} (Level Personal)</small></h1>
<ol class="breadcrumb">
  <li><a href="{{url('/admin')}}" class="btn-loading"><i class="fa fa-dashboard"></i> Home</a></li>
    <li><a href="Javascript:;"> Pembelian</a></li>
    <li><a href="{{url('/admin/pembelianmanual-produk/markup/role-personal')}}"> Produk</a></li>
  <li class="active">{{$kategori->product_name}} (Level Personal)</li>
</ol>
</section>
                    ...
                    <div class="col-md-12">
                       <div class="box">
                          <div class="box-header with-border">
                             <h3 class="box-title"><span class="hidden-xs">Produk {{$kategori->product_name}} (Level Personal)</span></h3>
                             <div class="box-tools pull-right">
                                  <a href="{{url('/admin/pembelianmanual-produk/markup/role-personal')}}" class="btn btn-primary btn-sm">Home Produk</a>
                              </div>
                          </div><!-- /.box-header -->
                          ...
                          <div style="text-align: center;margin-bottom: 20px;margin-top: 10px;">
                              <div id="desktop" class="hidden-xs hidden-sm">
                                  ...
                                  <a href="{{url('/admin/pembelianmanual-produk/markup/role-personal', $data->slug)}}" class="btn-loading btn btn-primary {{ url('/admin/pembelianmanual-produk/markup/role-personal', $data->slug) == request()->url() ? 'active' : '' }}" style="padding: 3px 7px;"><i class="fa fa-{{$data->icon}}" style="margin-right:5px;"></i> {{$data->product_name}}</a>
                                  @endforeach
                              </div>
                          </div>
                          ...
                          <h4 style="font-weight: bold;text-align: center;">{{$data->product_name}}</h4>
                            <div class="box-body table-responsive no-padding">
                               ...
                               <thead>
                                  ...
                                  @if(count($data->pembelianmanualproduk) > 0 && count($data->V_pembelianmanualproduk_personal) > 0)
                                      @foreach($data->V_pembelianmanualproduk_personal as $produk)
                                          <tr style="font-size: 13px;">
                                             ...
                                             @endif
                                             <td>
                                                <a href="{{url('/admin/pembelianmanual-produk/markup/role-personal/'.$kategori->slug.'/edit', $produk->id)}}" class="btn-loading btn btn-primary btn-sm" style="padding: 2px 5px;font-size: 10px;"><i class="fa fa-pencil-square-o fa-fw"></i>Ubah Harga</a>
                                             </td>
                                          </tr>
                                      @endforeach
                                  @else
                      ...
                      <div class="col-xs-12">
                          <div class="box">
                              <div class="box-header">
                                  <h3 class="box-title" style="font-size:18px;"><a href="{{url('/admin/pembelianmanual-produk/markup/role-personal')}}" class="hidden-lg btn-loading"><i class="fa fa-arrow-left" style="margin-right:10px;"></i></a>{{$kategori->product_name}}</h3>
                             <div class="box-tools pull-right">
                              ...
                              <div style="text-align: center;margin-bottom: 10px;margin-top: 10px;">
                                  <div id="mobile" class="hidden-lg hidden-md">
                                      @foreach($KategoriPembelianManual as $data)
                                      <a href="{{url('/admin/pembelianmanual-produk/markup/role-personal', $data->slug)}}" class="btn-loading btn btn-primary {{ url('/admin/pembelianmanual-produk/markup/role-personal', $data->slug) == request()->url() ? 'active' : '' }}" style="width:30px;padding: 3px 7px;"><i class="fa fa-{{$data->icon}}"></i><span class="hidden-xs hidden-sm" style="margin-left:5px;">{{$data->product_name}}</span></a>
                                      @endforeach
                                  </div>
                              </div>
                              @if(count($kategori->pembelianmanualoperator) > 0)
                                  ...
                                  <table class="table table-hover">
                                      ...
                                      <tr>
                                          ...
                                          <td align="right" style="width:35%;">
                                              ...
                                              @endif
                                              <div style="margin-top:5px;"><a href="{{url('/admin/pembelianmanual-produk/markup/role-personal/'.$kategori->slug.'/edit', $produk->id)}}" class="btn-loading btn btn-primary btn-sm" style="padding: 2px 5px;font-size: 10px;">Ubah Harga</a></div>
                                          </td>
                                      </tr>
                                      @endforeach
                                      @else
...
// $(document).ready(function() {
    ...
    $('#proses').on('click', function(){
       ...
       $.blockUI({ css: { 
        ...
        if(val_radio =='option_data'){
          $.ajax({
              type: "GET",
              url: '/admin/pembelianmanual-produk/markup/role-personal/plusminusmarkup?aksi='+aksi+'&nominal='+nominal+'type={{$kategori->id}}',
            ...
            $.ajax({
                type: "POST",
                url: "/admin/pembelianmanual-produk/markup/role-personal/plusminusmarkupAllData",
    ...
    {    
       ...
       $.ajax({
            type: "GET",
            url: "{{ url('admin/pembelianmanual-produk/markup/role-personal/cek-harga') }}",
            data: {'_token': '{{csrf_token()}}',id: id,url_link:url_link},
            success: function( msg ) {
              console.log('sgkwe',msg);
              if(msg == 'error'){
                $.unblockUI();
    ...
    function updateHargaSemua()
    {
      ...
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "GET",
            url: "{{ url('admin/pembelianmanual-produk/markup/role-personal/update-harga-semua') }}",
            success: function(response){
              if(response.success == true)
              {
                  $.unblockUI();
    ...
    function updateHargaPerOperator($kategori)
    {
      ...
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "GET",
            url: "/admin/pembelianmanual-produk/markup/role-personal/update-harga-peroperator?id_operator="+id_operator+"&id_kategori="+id_kategori+"",
            success: function(response){
              if(response.success == true)
              {
                  $.unblockUI();
    ...
    function updateHargaPerKategori(){
      ...
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "GET",
            url: "/admin/pembelianmanual-produk/markup/role-personal/update-harga-perkategori?id_kategori="+id_kategori+"",
            success: function(response){
              if(response.success == true)
              {
                  $.unblockUI();
    ...
    function sumMarkingByKategori()
    {
      ...
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "POST",
            url: "{{ url('admin/pembelianmanual-produk/markup/role-personal/update-harga-sum-markup-bykategori') }}",
    ...
    function sumMarkingByOperator()
    {
      ...
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "POST",
            url: "{{ url('admin/pembelianmanual-produk/markup/role-personal/update-harga-sum-markup-byoperator') }}",
    ...
    function updateByCheckbox(){
      ...
      for (index = 0, len = operator.length; index < len; ++index) {
        ...
        $.blockUI({ css: { 
          ...
          $.ajax({
              url: "{{ url('admin/pembelianmanual-produk/markup/role-personal/update-harga-perkchecklist') }}",
              method: "POST",
    ...
    function updateByCheckboxMarkup(){
      ...
      for (index = 0, len = operator.length; index < len; ++index) {
        ...
        $.blockUI({ css: { 
          ...
          $.ajax({
              url: "{{ url('admin/pembelianmanual-produk/markup/role-personal/update-harga-perkchecklist-markup') }}",
              method: "POST",
              data: {_token:"{{ csrf_token() }}",datachecked,jenis_produk:jenis_produk},
              success: function(response){
                if(response == 'error'){
  ...
  $('#pilih_kategori').on('change', function(e){
   ...
   $.ajaxSetup({
        ...
        $.ajax({
            url: '/admin/pembelianmanual-produk/markup/role-personal/process-cari/findproduct',
            dataType: "json",
            type: "GET",
            data: {
