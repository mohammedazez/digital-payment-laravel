...
<h1>Produk <small>{{$kategori->product_name}} (Level Agen)</small></h1>
<ol class="breadcrumb">
  <li><a href="{{url('/admin')}}" class="btn-loading"><i class="fa fa-dashboard"></i> Home</a></li>
    <li><a href="Javascript:;"> Pembelian Manual</a></li>
    <li><a href="{{url('/admin/pembelianmanual-produk/markup/role-agen')}}"> Produk</a></li>
  <li class="active">{{$kategori->product_name}} (Level Agen)</li>
</ol>
</section>
                    ...
                    <div class="col-md-12">
                       <div class="box">
                          <div class="box-header with-border">
                             <h3 class="box-title"><span class="hidden-xs">Produk {{$kategori->product_name}} (Level Agen)</span></h3>
                             <div class="box-tools pull-right">
                                  <a href="{{url('/admin/pembelianmanual-produk/markup/role-agen')}}" class="btn btn-primary btn-sm">Home Produk</a>
                              </div>
                          </div><!-- /.box-header -->
                          
                          <div style="text-align: center;margin-bottom: 20px;margin-top: 10px;">
                              <div id="desktop" class="hidden-xs hidden-sm">
                                  @foreach($KategoriPembelianManual as $data)
                                  <a href="{{url('/admin/pembelianmanual-produk/markup/role-agen', $data->slug)}}" class="btn-loading btn btn-primary {{ url('/admin/pembelianmanual-produk/markup/role-agen', $data->slug) == request()->url() ? 'active' : '' }}" style="padding: 3px 7px;"><i class="fa fa-{{$data->icon}}" style="margin-right:5px;"></i> {{$data->product_name}}</a>
                                  @endforeach
                              </div>
                          </div>
                          ...
                          @if(count($kategori->pembelianmanualoperator) > 0)
                          @foreach($kategori->pembelianmanualoperator as $data)
                          <h4 style="font-weight: bold;text-align: center;">{{$data->product_name}}</h4>
                            <div class="box-body table-responsive no-padding">
                               ...
                               <thead>
                                  ...
                                  @if(count($data->pembelianmanualproduk) > 0 && count($data->V_pembelianmanualproduk_agen))
                                      @foreach($data->V_pembelianmanualproduk_agen as $produk)
                                      <tr style="font-size: 13px;">
                                             ...
                                             @endif
                                             <td>
                                                <a href="{{url('/admin/pembelianmanual-produk/markup/role-agen/'.$kategori->slug.'/edit', $produk->id)}}" class="btn-loading btn btn-primary btn-sm" style="padding: 2px 5px;font-size: 10px;"><i class="fa fa-pencil-square-o fa-fw"></i>Ubah Harga</a>
                                             </td>
                                          </tr>
                                      @endforeach
                                  @else
                                    <tr>
                                        <td colspan="10" style="font-style:italic;text-align:center;background-color:#F3F3F3;">Data Produk tidak ditemukan</td>
                                    </tr>
                                  @endif
                               </table>
                            </div><!-- /.box-body -->
                      ...
                      <div class="col-xs-12">
                          <div class="box">
                              <div class="box-header">
                                  <h3 class="box-title" style="font-size:18px;"><a href="{{url('/admin/pembelianmanual-produk/markup/role-agen')}}" class="hidden-lg btn-loading"><i class="fa fa-arrow-left" style="margin-right:10px;"></i></a>{{$kategori->product_name}}</h3>
                             <div class="box-tools pull-right">
                              ...
                              <div style="text-align: center;margin-bottom: 10px;margin-top: 10px;">
                                  <div id="mobile" class="hidden-lg hidden-md">
                                      @foreach($KategoriPembelianManual as $data)
                                      <a href="{{url('/admin/pembelianmanual-produk/markup/role-agen', $data->slug)}}" class="btn-loading btn btn-primary {{ url('/admin/pembelianmanual-produk/markup/role-agen', $data->slug) == request()->url() ? 'active' : '' }}" style="width:30px;padding: 3px 7px;"><i class="fa fa-{{$data->icon}}"></i><span class="hidden-xs hidden-sm" style="margin-left:5px;">{{$data->product_name}}</span></a>
                                      @endforeach
                                  </div>
                              </div>
                              @if(count($kategori->pembelianmanualoperator) > 0)
                              ...
                              <div class="box-body" style="padding: 0px">
                                  <table class="table table-hover">
                                      @if(count($data->pembelianmanualproduk) > 0)
                                      @foreach($data->pembelianmanualproduk as $produk)
                                      <tr>
                                          <td>
                                              <div><small>ID : #{{$produk->id}}</small></div>
                                              <div style="font-size: 14px;font-weight: bold;">{{$produk->pembelianmanualoperator->product_name}} - {{$produk->product_name}}</div>
                                              <div>{{$produk->pembelianmanualkategori->product_name}}</div>
                                          </td>
                                          <td align="right" style="width:35%;">
                                              ...
                                              @endif
                                              <div style="margin-top:5px;"><a href="{{url('/admin/pembelianmanual-produk/markup/role-agen/'.$kategori->slug.'/edit', $produk->id)}}" class="btn-loading btn btn-primary btn-sm" style="padding: 2px 5px;font-size: 10px;">Ubah Harga</a></div>
                                          </td>
                                      </tr>
                                      @endforeach
                                      @else
                  ...
                  <div class="col-md-12">
                     <div class="box">
                        ...
                        <div class="box-header with-border">
                          <!-- price default -->
                          <div class="col-md-4">
                            <div class="panel-heading">Reset All Markup</div>
                          ...
                          <!-- price markup -->
                          <div class="col-md-4">
                            <div class="panel-heading">Reset Makup Data By Optional</div>
                            <div class="panel panel-default">
                              <div class="panel-body text-center">
                                <div class="form-group">
...
// $(document).ready(function() {
    ...
    $('#proses').on('click', function(){
       ...
       $.blockUI({ css: { 
        ...
        if(val_radio =='option_data'){
          $.ajax({
              url: '/admin/pembelianmanual-produk/markup/role-agen/plusminusmarkup?aksi='+aksi+'&nominal='+nominal+'type={{$kategori->id}}',
              type: "GET",
          ...
          $.ajax({
                type: "POST",
                url: "/admin/pembelianmanual-produk/markup/role-agen/plusminusmarkupAllData",
    ...
    function cekHargaProduk($id, $url_link)
    {    
       var id = $id;
       ...
       $.ajax({
            type: "GET",
            url: "{{ url('admin/pembelianmanual-produk/markup/role-agen/cek-harga') }}",
            data: {'_token': '{{csrf_token()}}',id: id,url_link:url_link},
            success: function( msg ) {
              if(msg == 'error'){
                $.unblockUI();
                swal("Error!", "Checking gagal,Api tidak terhubung!", "error");
                return false;
              }
                var dataku=[];
                $.each(msg, function(index, produkObj){
                    var data_hrg = produkObj.price_api;
                    var data_desc = produkObj.description;
                      dataku.push({data_hrg,data_desc});
                });
              swal({
                  title: "Harga pada api "+dataku[0].data_hrg+"",
                  type: "info",
    ...
    function updateHargaSemua()
    {
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "GET",
            url: "{{ url('admin/pembelianmanual-produk/markup/role-agen/update-harga-semua') }}",
            success: function(response){
              if(response.success == true)
              {
                  $.unblockUI();
    ...
    function updateHargaPerOperator($kategori)
    {
      ...
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "GET",
            url: "/admin/pembelianmanual-produk/markup/role-agen/update-harga-peroperator?id_operator="+id_operator+"&id_kategori="+id_kategori+"",
            success: function(response){
              if(response.success == true)
              {
                  $.unblockUI();
    ...
    function updateHargaPerKategori(){
      ...
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "GET",
            url: "/admin/pembelianmanual-produk/markup/role-agen/update-harga-perkategori?id_kategori="+id_kategori+"",
            success: function(response){
              if(response.success == true)
              {
                  $.unblockUI();
    ...
    function sumMarkingByKategori()
    {
      ...
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "POST",
            url: "{{ url('admin/pembelianmanual-produk/markup/role-agen/update-harga-sum-markup-bykategori') }}",
    ...
    function sumMarkingByOperator()
    {
      ...
      $.blockUI({ css: { 
       ...
       $.ajax({
            type: "POST",
            url: "{{ url('admin/pembelianmanual-produk/markup/role-agen/update-harga-sum-markup-byoperator') }}",
  ...
  $(function () {
      var index, len;
      var operator = <?php echo json_encode($kategori->pembelianmanualoperator);?>;
    ...
    function updateByCheckbox(){
      ...
      for (index = 0, len = operator.length; index < len; ++index) {
        ...
        $.blockUI({ css: { 
          ...
          $.ajax({
              url: "{{ url('admin/pembelianmanual-produk/markup/role-agen/update-harga-perkchecklist') }}",
              method: "POST",
    ...
    function updateByCheckboxMarkup(){
      ...
      for (index = 0, len = operator.length; index < len; ++index) {
        ...
        $.blockUI({ css: { 
          ...
          $.ajax({
              url: "{{ url('admin/pembelianmanual-produk/markup/role-agen/update-harga-perkchecklist-markup') }}",
              method: "POST",
              data: {_token:"{{ csrf_token() }}",datachecked,jenis_produk:jenis_produk},
              success: function(response){
                if(response == 'error'){
  ...
  $('#pilih_kategori').on('change', function(e){
   ...
   $.ajaxSetup({
        ...
        $.ajax({
            url: '/admin/pembelianmanual-produk/markup/role-agen/process-cari/findproduct',
            dataType: "json",
            type: "GET",
            data: {
