        ...
        <div class="col-md-6">
            <div class="box box-green">
                <div class="box-header">
                    <h3 class="box-title"><a href="{{route('pembelian-operator.index')}}" class="hidden-lg btn-loading"><i class="fa fa-arrow-left" style="margin-right:10px;"></i></a>Ubah Operator</h3>
                </div>
                <form role="form" action="{{route('pembelian-operator.update', $operators->id)}}" method="post">
                    ...
                    <div class="box-body">
                        <div class="form-group{{ $errors->has('kategori') ? ' has-error' : '' }}">
                            ...
                            <select name="kategori" class="form-control">
                                ...
                                <option value="{{$data->id}}" {{ $operators->pembeliankategori_id == $data->id ? 'selected' : '' }}>{{$data->product_name}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group{{ $errors->has('product_id') ? ' has-error' : '' }}">
                            ...
                            <input type="text" class="form-control" name="product_id" value="{{$operators->product_id ?? old('product_id')}}"  placeholder="Masukkan ID Operator Server">
                            {!! $errors->first('product_id', '<p class="help-block"><small>:message</small></p>') !!}
                        </div>
                        <div class="form-group{{ $errors->has('product_name') ? ' has-error' : '' }}">
                            <label>Nama Operator : </label>
                            <input type="text" class="form-control" name="product_name" value="{{$operators->product_name ?? old('product_name')}}"  placeholder="Masukkan Nama Operator">
                            {!! $errors->first('product_name', '<p class="help-block"><small>:message</small></p>') !!}
                        </div>
                        <div class="form-group{{ $errors->has('prefix') ? ' has-error' : '' }}">
                            <label>Nomor Prefix : </label>
                            <input type="text" class="form-control" name="prefix" value="{{$operators->prefix ?? old('prefix')}}" placeholder="Masukkan Nomor Prefix Operator">
                            {!! $errors->first('prefix', '<p class="help-block"><small>:message</small></p>') !!}
                        </div>
                        <div class="form-group{{ $errors->has('img_url') ? ' has-error' : '' }}">
                            <label>URL Gambar : </label>
                            <input type="text" class="form-control" name="img_url" value="{{$operators->img_url ?? old('img_url')}}"  placeholder="Masukkan URL Gambar Operator">
                            {!! $errors->first('img_url', '<p class="help-block"><small>:message</small></p>') !!}
                        </div>
                        <div class="form-group{{ $errors->has('status') ? ' has-error' : '' }}">
                            <label>Status Operator : </label>
                            <select name="status" class="form-control">
                                <option value="1" {{ $operators->status == 1 ? 'selected' : '' }}>AKTIF</option>
                                <option value="0" {{ $operators->status == 0 ? 'selected' : '' }}>TIDAK AKTIF</option>
                            </select>
            ...
            <div class="box box-solid box-penjelasan">
                ...
                <div class="box-body">
                    ...
                    <table class="table table-hover" style="font-size: 13px;">
                        ...
                        <?php $no=1; ?>
                        @if($operatorsall->count() > 0)
                        @foreach($operatorsall as $data)
                        <tr>
                            <td>{{$no++}}</td>
