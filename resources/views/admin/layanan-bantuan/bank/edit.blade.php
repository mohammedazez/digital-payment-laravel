               ...
               {{csrf_field()}}
                  <div class="box-body">
                     <div class="form-group{{ $errors->has('nama_bank') ? ' has-error' : '' }}">
                        ...
                        <input type="text" class="form-control" name="nama_bank" value="{{ $banks->nama_bank ?? old('nama_bank') }}"  placeholder="Masukkan Nama Bank">
                        {!! $errors->first('nama_bank', '<p class="help-block"><small>:message</small></p>') !!}
                     </div>
                     <div class="form-group{{ $errors->has('atas_nama') ? ' has-error' : '' }}">
                        <label>Nama Pemilik : </label>
                        <input type="text" class="form-control" name="atas_nama" value="{{ $banks->atas_nama ?? old('atas_nama') }}"  placeholder="Masukkan Nama Pemilik Rekening">
                         {!! $errors->first('atas_nama', '<p class="help-block"><small>:message</small></p>') !!}
                     </div>
                     <div class="form-group{{ $errors->has('no_rek') ? ' has-error' : '' }}">
                        <label>Nomor Rekening : </label>
                        <input type="text" class="form-control" name="no_rek" value="{{ $banks->no_rek ?? old('no_rek') }}"  placeholder="Masukkan Nomor Rekening Rekening">
                         {!! $errors->first('no_rek', '<p class="help-block"><small>:message</small></p>') !!}
                     </div>
                     <div class="form-group{{ $errors->has('bank_id') ? ' has-error' : '' }}">
                        <label>Bank ID : </label>
                        <input type="text" class="form-control" name="bank_id" value="{{ $banks->bank_id ?? old('bank_id') }}"  placeholder="Masukkan Bank ID">
                         {!! $errors->first('bank_id', '<p class="help-block"><small>:message</small></p>') !!}
                     </div>
                     <div class="form-group{{ $errors->has('image') ? ' has-error' : '' }}">
                        <label>Logo / Gambar Bank : </label>
                        <input type="file" name="image" accept="image/*">
                        <img src="{{asset('img/'.$banks->image)}}" class="img-thumbnail" style="height: 40px;margin-top: 5px;"  alt="{{$banks->nama_bank}}">
