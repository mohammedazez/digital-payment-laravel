         ...
         <div class="col-md-6">
            <div class="box box-default">
               <div class="box-header">
                 <h3 class="box-title"><a href="{{route('faqs.index')}}" class="hidden-lg btn-loading"><i class="fa fa-arrow-left" style="margin-right:10px;"></i></a>Ubah FAQ</h3>
               </div>
               <form role="form" action="{{route('faqs.update', $faqs->id)}}" method="post">
               <input type="hidden" name="_method" value="PATCH">
               {{csrf_field()}}
                  <div class="box-body">
                     <div class="form-group{{ $errors->has('pertanyaan') ? ' has-error' : '' }}">
                        <label>Pertanyaan : </label>
                        <input type="text" class="form-control" name="pertanyaan" value="{{ $faqs->pertanyaan ?? old('pertanyaan') }}"  placeholder="Masukkan Pertanyaan">
                        {!! $errors->first('pertanyaan', '<p class="help-block"><small>:message</small></p>') !!}
                     </div>
                     <div class="form-group{{ $errors->has('jawaban') ? ' has-error' : '' }}">
                        <label>Jawaban : </label>
                        <textarea name="jawaban" class="form-control" rows="5" placeholder="Masukkan Jawaban">{{ $faqs->jawaban ?? old('jawaban') }}</textarea>
