...
<section class="content-header">
   ...
   <ol class="breadcrumb">
      ...
      <li class="active">Ubah Terms of Service</li>
   </ol>
   </section>
         ...
         <div class="col-md-7">
            <div class="box box-default">
               <div class="box-header">
                 <h3 class="box-title"><a href="{{route('tos.index')}}" class="hidden-lg btn-loading"><i class="fa fa-arrow-left" style="margin-right:10px;"></i></a>Ubah Terms of Service</h3>
               </div>
               <form role="form" action="{{route('tos.update', $tos->id)}}" method="post">
               <input type="hidden" name="_method" value="PATCH">
               {{csrf_field()}}
                  <div class="box-body">
                     <div class="form-group{{ $errors->has('title') ? ' has-error' : '' }}">
                        <label>Title : </label>
                        <input type="text" class="form-control" name="title" value="{{ $tos->title ?? old('title') }}"  placeholder="Masukkan Title">
                        {!! $errors->first('title', '<p class="help-block"><small>:message</small></p>') !!}
                     </div>
                     <div class="form-group{{ $errors->has('content') ? ' has-error' : '' }}">
                        <label>Isi TOS/Content : </label>
                        <textarea class="textarea" name="content" placeholder="Masukkan Content atau isi TOS" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;">{{ $tos->content ?? old('content') }}</textarea>
